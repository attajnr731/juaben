...
const Avatar = ({ src, name, size = "md" }) => {
  ...
  const sizeClass = size === "lg" ? "w-20 h-20 text-xl" : size === "md" ? "w-12 h-12 text-sm" : "w-10 h-10 text-xs";
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border-2 border-[#1a3a6e] shadow-xl px-4 py-3">
        <p className="text-[#1a3a6e] font-black text-sm">{payload[0].payload.name}</p>
    ...
    return (
      <div className="min-h-screen bg-gray-50 px-4 md:px-8 py-8">
        <div className="max-w-5xl mx-auto text-center py-16">
          <p className="text-[#1a3a6e] font-black text-xl mb-2">No Results Available</p>
        ...
        <div className="px-4 md:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-[9px] tracking-widest uppercase">Juaben SHS 2026</p>
                </Link>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-4 text-xs">
              <div className="flex items-center gap-1.5">
                <HowToVoteOutlinedIcon style={{ fontSize: 16 }} className="text-gray-400" />
                <span className="text-gray-600"><span className="font-black text-[#1a3a6e]">{totalVotes}</span> votes</span>
              </div>
              <div className="flex items-center gap-1.5">
                <PeopleOutlineOutlinedIcon style={{ fontSize: 16 }} className="text-gray-400" />
                <span className="text-gray-600"><span className="font-black text-[#1a3a6e]">{turnout}%</span> turnout</span>
              </div>
            </div>
            <button onClick={refresh} disabled={refreshing}
              className="p-2 rounded hover:bg-gray-100 text-gray-400 hover:text-[#1a3a6e] transition-colors disabled:opacity-50">
              <RefreshOutlinedIcon style={{ fontSize: 20 }} className={refreshing ? "animate-spin" : ""} />
            </button>
          </div>
        </div>
              ...
              <div className="w-full max-w-7xl">
                ...
                {!hasVotes ? (
                  ...
                  <div className="text-center py-16">
                    <p className="text-gray-300 text-sm">No votes cast for this position yet.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
                    ...
                    <div className="bg-white border-2 border-[#c8a84b] shadow-xl p-8 flex flex-col items-center text-center gap-4">
                      <div className="w-16 h-16 rounded-full bg-[#FFD700] flex items-center justify-center shadow-lg">
                        <EmojiEventsOutlinedIcon style={{ fontSize: 32 }} className="text-white" />
                      </div>
                      <div>
                        <p className="text-[#c8a84b] text-xs tracking-widest uppercase font-bold mb-2">Winner</p>
                        <Avatar src={result.winner?.profilePicture} name={result.winner?.name} size="lg" />
                        <p className="text-[#1a3a6e] font-black text-2xl mt-4 mb-1">{result.winner?.name}</p>
                        <p className="text-gray-400 text-xs mb-4">{result.position}</p>
                        <div className="flex items-center justify-center gap-2 bg-[#1a3a6e] text-white px-6 py-2">
                          ...
                          <span className="font-black text-lg">{result.winner?.voteCount || 0}</span>
                          <span className="text-xs uppercase tracking-widest">Votes</span>
                        </div>
                        {result.totalVotes > 0 && (
                          <p className="text-gray-400 text-xs mt-3">
                            {Math.round(((result.winner?.voteCount || 0) / result.totalVotes) * 100)}% of total votes
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-gray-100 shadow-sm p-6 flex flex-col">
                      <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-100">
                        <WorkOutlineOutlinedIcon style={{ fontSize: 18 }} className="text-[#1a3a6e]" />
                        <p className="text-[#1a3a6e] font-black text-sm uppercase tracking-wide">Vote Distribution</p>
                      </div>
                ...
                {hasVotes && (
                  <div className="bg-white border border-gray-100 shadow-sm overflow-hidden mb-10">
                    <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
                      <p className="text-[#1a3a6e] font-black text-sm uppercase tracking-wide">All Candidates</p>
                    </div>
                    <div className="divide-y divide-gray-50">
                      {result.candidates.map((c, i) => {
                        const percentage = result.totalVotes > 0
                          ? Math.round(((c.voteCount || 0) / result.totalVotes) * 100)
                          : 0;
                        return (
                          <div key={c._id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
                            <div className="flex items-center gap-4 flex-1 min-w-0">
                              ...
                              <Avatar src={c.profilePicture} name={c.name} size="sm" />
                              <div className="flex-1 min-w-0">
                                <p className="text-gray-700 font-bold text-sm truncate">{c.name}</p>
                                {c.bio && (
                                  <p className="text-gray-400 text-xs truncate">{c.bio}</p>
                                )}
                              </div>
                            </div>
                            <div className="flex items-center gap-6 shrink-0">
                              <div className="text-right">
                                <p className="text-[#1a3a6e] font-black text-xl">{c.voteCount || 0}</p>
                ...
                <div className="flex justify-center">
                  {!isLast ? (
                    ...
                    <div className="text-center">
                      <p className="text-gray-400 text-sm mb-2">End of Results</p>
                      <p className="text-[#1a3a6e] font-black text-xs uppercase tracking-widest">
                        {positions.length} Position{positions.length !== 1 ? "s" : ""} · {totalVotes} Total Votes
                      </p>
                    </div>
                  )}
                </div>
