...
const Settings = () => {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      const otpRes = await fetch(`${API}/settings/otps`, {
        headers: authHeaders(),
      });
      const otpData = await otpRes.json();
      setOtpList(otpData.otps || []);
    } catch (err) {
  ...
  const generateOtp = async () => {
    ...
    try {
      ...
      // refresh list
      const listRes = await fetch(`${API}/settings/otps`, {
        headers: authHeaders(),
      });
      const listData = await listRes.json();
      setOtpList(listData.otps || []);
    } catch (err) {
      setOtpStatus({
        status: "error",
  ...
  // ── Handler: clear used OTPs ──
  const clearUsedOtps = async () => {
    setClearLoading(true);
    try {
      await fetch(`${API}/settings/otps/used`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      const listRes = await fetch(`${API}/settings/otps`, {
      ...
      const listData = await listRes.json();
      setOtpList(listData.otps || []);
      setOtpStatus({ status: "success", message: "Used OTPs cleared." });
      setTimeout(() => setOtpStatus({ status: "", message: "" }), 3000);
    } catch {
      setOtpStatus({ status: "error", message: "Failed to clear." });
    } finally {
